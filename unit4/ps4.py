...
def r_squared(y, estimated):
    ...
    sumSqRes = sum([(y[i] - estimated[i])**2 for i in range(len(y))])
    
    totSumSq = sum([(y[i] - np.mean(y))**2 for i in range(len(y))])
    
    return 1.0 - sumSqRes/totSumSq
...
def evaluate_models_on_training(x, y, models):
    ...
    for i in range(len(models)):
        ...
        pylab.title("Yearly Average Temperatures")
